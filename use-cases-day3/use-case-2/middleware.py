        ...
        if last_message and last_message.text:
            ...
            if not classification.get("safe", True):
                ...
                context.result = AgentResponse(
                    messages=[Message(role="assistant", text=refusal)]
                )
        ...
        if context.result and context.result.messages:
            ...
            if response_text:
                ...
                if not classification.get("safe", True):
                    ...
                    context.result = AgentResponse(
                        messages=[
                            Message(role="assistant", text=self.FALLBACK_MESSAGE)
                        ]
                    )
                    return
...
class ExceptionHandlingMiddleware(AgentMiddleware):
    ...
    async def process(
        ...
        try:
            ...
            context.result = AgentResponse(
                messages=[Message(role="assistant", text=self.POLISHED_MESSAGE)]
            )
